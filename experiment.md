...
1. **Ours: Diffusion Planner + 全部优化**
   当前 diffusion planner 作为我们的完整系统评估。评估时打开已经确认有用的优化，例如 world-model rerank、refinement、corrective replan / learned correction。它不是消融对象，而是一个完整方法。
...
正式结果按 `task x method` 汇总 mean / std，并写入 `result.md`。
...
| `reacher` | `eval_profile=diffusion diffusion_selection_mode=wm_only diffusion_refinement.enabled=true`；如果已确认 corrective 对 Reacher 有稳定收益，再纳入 |
| `pusht` | 优先 `eval_profile=corrective_learned diffusion_selection_mode=wm_only diffusion_refinement.enabled=true`；如果 corrector bundle 不稳定，则用 `eval_profile=corrective_replan` |
...
如果 PushT 的 learned corrector bundle 不可用，切换为 corrective replan：

```bash
scripts/run_comparison_experiments.sh --pusht-ours-profile corrective_replan
```
...
如果 `corrective_learned` bundle 不可用，PushT 的 first-stage fallback 是：
...
`result.md` 同时保留 raw run 表和 summary 表。最终主表按 `task x method` 聚合 mean / std。
...
- anchor construction
- `diffusion_refinement.enabled`
- `corrective.error_threshold`
- `corrective.correction_interval`
- `corrective.execute_horizon`
